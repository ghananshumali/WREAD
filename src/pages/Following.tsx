...
const Following = () => {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/user/following/${id}`)
      .then(res => res.json())
    ...
    <div className="container mx-auto max-w-xl px-4 py-10 md:px-6">
      ...
      {users.length > 0 ? (
        ...
        <div className="py-20 text-center space-y-3">
          <UserCheck className="mx-auto h-10 w-10 text-muted-foreground/20" />
          <p className="font-display text-xl text-muted-foreground/40">Not following anyone yet</p>
        </div>
      )}
    </div>
  );
